...
void Draw3dGuiCube(glm::mat4& Pvr, glm::mat4& Vvr, glm::mat4& Mvr)
{
   ...
   //Fade if mouse is outside UI
   SetCurrentContextToActiveTexture();
   if (ImGui::IsMouseHoveringAnyWindow())
   {
      BaseColor = glm::vec4(1.0f);
   }
   ...
   if(pTutorial->tutorialMode == true)
   {
      rot_angle = -0.5f;
   }
   ...
   const int controller = 1; //0 = left, 1=right
	float current_drag_x = ImGui_Impl_VR_GetAxisDragDelta(controller).x;

   if (!ImGui::IsMouseHoveringAnyWindow() || pTutorial->tutorialMode == true) //only rotate when mouse is hovering window or not in tutorial mode
   {
      current_drag_x = 0.0f;
      last_drag_x = 0.0f;
   }
