...
extern TrackedCamera* ptr_tracked_camera;
...
bool CMainApplication::BInit()
{
	...
	m_pWindow = SDL_CreateWindow( "AtlasVR", nWindowPosX, nWindowPosY, m_nWindowWidth, m_nWindowHeight, unWindowFlags );
	if (m_pWindow == NULL)
	{
...
{
	if (severity != GL_DEBUG_SEVERITY_NOTIFICATION) //disable printing of notifications
	{
		dprintf("GL Error: %s\n", message);
	}
}
...
bool CMainApplication::HandleInput()
{
	...
	while ( SDL_PollEvent( &sdlEvent ) != 0 )
	{
		...
		{
			vr_keydown(sdlEvent.key.keysym.sym);
	...
	for( vr::TrackedDeviceIndex_t unDevice = 0; unDevice < vr::k_unMaxTrackedDeviceCount; unDevice++ )
	{
		...
		if( m_pHMD->GetControllerState( unDevice, &state ) )
		{
			...
			ImGui_Impl_VR_ProcessController(unDevice, &state, pose);
...
void CMainApplication::RunMainLoop()
{
	...
	io.Fonts->AddFontFromFileTTF("include/imgui/extra_fonts/Cousine-Regular.ttf", 24);
	vr_init(m_pHMD);
	...
	m_trackedCamera.InitTrackedCamera(m_pHMD);
	ptr_tracked_camera = &m_trackedCamera;
...
void CMainApplication::RenderFrame()
{
	...
	if ( m_pHMD )
	{
		Draw2dVRGui(); // renders gui to texture (don't do for each eye)
...
bool CMainApplication::CreateAllShaders()
{
	...
	m_unRenderModelProgramID = CompileGLShader( 
		...
		"void main()\n"
		"{\n"
		"   outputColor = 3.0*texture( diffuse, v2TexCoord);\n"
		"}\n"
...
void CMainApplication::DrawControllers()
{
   ...
   if (hide_pointer == true)
   {
	...
	for ( vr::TrackedDeviceIndex_t unTrackedDevice = vr::k_unTrackedDeviceIndex_Hmd + 1; unTrackedDevice < vr::k_unMaxTrackedDeviceCount; ++unTrackedDevice )
	{
      ...
      //Don't draw lines for left controller
      if (unTrackedDevice == m_pHMD->GetTrackedDeviceIndexForControllerRole(vr::TrackedControllerRole_LeftHand))
         continue;
		...
		for ( int i = 0; i < 3; ++i )
		{
			...
			point[i] += 0.005f;  // offset in X, Y, Z
			//color[i] = 1.0;  // R, G, B
			point = mat * point;
...
{
   const int nsamples = 1; //was 4
	glGenFramebuffers(1, &framebufferDesc.m_nRenderFramebufferId );
	glBindFramebuffer(GL_FRAMEBUFFER, framebufferDesc.m_nRenderFramebufferId);

	glGenRenderbuffers(1, &framebufferDesc.m_nDepthBufferId);
	glBindRenderbuffer(GL_RENDERBUFFER, framebufferDesc.m_nDepthBufferId);
	glRenderbufferStorageMultisample(GL_RENDERBUFFER, nsamples, GL_DEPTH_COMPONENT, nWidth, nHeight );
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER,	framebufferDesc.m_nDepthBufferId );

   
	glGenTextures(1, &framebufferDesc.m_nRenderTextureId );
	glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, framebufferDesc.m_nRenderTextureId );
	glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, nsamples, GL_RGBA8, nWidth, nHeight, true);
...
void CMainApplication::RenderStereoTargets()
{
	...
	//glEnable( GL_MULTISAMPLE ); //TEMP disabled to prevent outline artifact when raycasting
...
void CMainApplication::RenderScene( vr::Hmd_Eye nEye, GLuint fbo)
{
   ...
   glm::mat4 M;
	...
	if( nEye == vr::Eye_Left )
	{
		P = Matrix4ToGlm(m_mat4ProjectionLeft);
		V = Matrix4ToGlm(m_mat4eyePosLeft);
		M = Matrix4ToGlm(m_mat4HMDPose);

      if (lock_3p_camera == false)
      {
         P3p[0] = P;
         V3p[0] = V;
         M3p[0] = M;
	...
	{
		P = Matrix4ToGlm(m_mat4ProjectionRight);
		V = Matrix4ToGlm(m_mat4eyePosRight);
		M = Matrix4ToGlm(m_mat4HMDPose);
	...
	if( !bIsInputCapturedByAnotherProcess)
	{
      ...
      else
      {
         ...
         if (nEye == vr::Eye_Left)
         {
            PVM3p = P3p[0]*V3p[0]*M3p[0];
         }
         else
         {
            PVM3p = P3p[1] * V3p[1] * M3p[1];
         }
         
         glUniformMatrix4fv(m_nControllerMatrixLocation, 1, GL_FALSE, glm::value_ptr(PVM3p));
      }
		
      if (hide_pointer == false)
      {
         glLineWidth(1.5f);
         glEnable(GL_BLEND);
   ...
   Draw3dGui(P, V, M); //render gui last for correct blending
	...
	for( uint32_t unTrackedDevice = 0; unTrackedDevice < vr::k_unMaxTrackedDeviceCount; unTrackedDevice++ )
	{
      ...
      else
      {
         ...
         if (nEye == vr::Eye_Left)
         {
            PVM3p = P3p[0]*V3p[0]*M3p[0]*Matrix4ToGlm(matDeviceToTracking);
         }
         else
         {
            PVM3p = P3p[1] * V3p[1] * M3p[1] * Matrix4ToGlm(matDeviceToTracking);
         }
         glUniformMatrix4fv(m_nRenderModelMatrixLocation, 1, GL_FALSE, glm::value_ptr(PVM3p));
      }

	
      if (hide_controllers == false)
      {
         if (m_pHMD->GetTrackedDeviceClass(unTrackedDevice) != vr::ETrackedDeviceClass::TrackedDeviceClass_TrackingReference)
         {
